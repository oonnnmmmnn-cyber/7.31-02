...
### 9.10 操作按钮容器（Divider Block / Action Bar / Footer Block）
...
**批量操作与工具栏（Action Bar，对应 §12.1 卡槽 ④）：**
- **位置**：位于数据表格**上方**（筛选区 / 业务提示条之下），与表头紧邻
- **内容**：左侧为「新建」「批量导出」「锁定」等业务按钮组；勾选表格行后，「已选择 n 项」汇总文案及关联批量操作（如「批量执行」「取消选择」）**同样展示在本区域**，而非底部
- **不使用** `footer-block` 类名；与分页吸底栏职责分离

**底部固定分页栏（Footer Block，`footer-block`）：**
- **职责**：`footer-block` 吸底承载的主要是**分页器（Pagination）**，不是批量操作栏
- `position: sticky; bottom: 0; z-index: 101`，白底
- padding `12px 0 0 12px`（或 `12px 20px`，视页面内边距而定）
- 左右各 `margin: -16px`（撑满父容器内边距，新架构 temu-oms-newon）
- **投影**：向上投射轻阴影 `box-shadow: 0px -4px 12px~24px rgba(0,0,0,.12)`，用于在内容滚动时区分固定分页层与表格内容
- **内容**：「共有 N 条」总数文案 + 页码切换 + 每页条数选择 + 跳页输入，规格见 §5.17

> **产图要点**：不要把「已选择 n 项」类批量操作放进 `footer-block`；批量操作在表头上方 Action Bar，分页在表格下方 Footer Block。
...
- 批量操作工具栏（Action Bar）：位于表格上方，含业务按钮及「已选择 n 项」汇总，不使用 footer-block
- 底部分页栏（footer-block）：sticky 置底 z-index 101，白底，承载 Pagination，带向上轻投影
...
- 信息层级用黑色透明度梯度而非彩色区分
- 业务页面铺满框架（无额外内边距），底部分页栏（footer-block）常驻吸底
...
|    - 勾选后: [已选择 n 项] [取消选择] [批量执行] 等汇总操作亦在本区域       |
|    - 右侧: [一键复制所选 ID] [查询申诉记录]                             |
+-----------------------------------------------------------------------+
| 5. 数据表格区 (Table Region)                                          |
|    - 表头: 背景 #f5f5f5 | 高度: 36px~40px | Cell Padding: 9px 12px    |
|    - 行高: 随内容扩展 (支持展示商品图文/SKU 标签)                       |
+-----------------------------------------------------------------------+
| 6. 底部分页栏 (Footer Block · footer-block · sticky)                    |
|    - 承载 Pagination，非批量操作栏                                      |
|    - 分页器: 高度 28px, 右对齐, 「共有 N 条」靠左                        |
|    - sticky bottom + 向上轻投影，与表格内容滚动分离                      |
+-----------------------------------------------------------------------+
...
##### ④ 批量操作与工具栏 (Action Bar)
* **排列格式**：位于筛选区下方、**数据表格上方**（紧贴表头之上）。与上/下区块保持 **`10px` ~ `16px`** 的外间距（Margin）。**不使用 `footer-block` 类名，不吸底。**
* **对齐逻辑**：
  * **左侧**：强业务操作按钮组（如“新增”、“批量导出”、“锁定”），按钮间距 `10px`。
  * **勾选联动**：表格行被勾选后，「已选择 n 项」文案及「取消选择」「批量执行」等批量操作**展示在本 Action Bar 区域**（可替换或追加在左侧按钮组旁），**不要**放到底部 `footer-block`。
...
##### ⑥ 底部分页栏 (Footer Block · `footer-block`)
* **职责定位**：`footer-block` 是表格下方的**吸底分页容器**，主要承载 **Pagination 分页器**，**不是**「已选择 n 项」类批量操作栏。
* **布局规格**：
  * `position: sticky; bottom: 0; z-index: 101`，白底，随表格区域滚动后吸附在可视区域底部。
  * 带向上投射轻阴影，与上方可滚动表格内容形成层级分离（详见 §9.10）。
* **分页器 (Pagination)**：
  * **高度**：**`28px`**（Medium 规格）。
  * **对齐方式**：`justify-content: flex-end`（页码区靠右）；「共有 N 条」总数文案靠左（`margin-right: auto`）。
  * **外边距**：与表格紧贴（Margin-Top 为 `0px`），整体作为 footer-block 内唯一主内容。
